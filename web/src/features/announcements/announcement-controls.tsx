...
import { canCreateAnnouncement, type DashboardRole } from './permissions'
...
export function Announcements({ announcements, viewerRole }: { announcements: AnnouncementRecord[]; viewerRole: DashboardRole }) {
  ...
  const canCreate = canCreateAnnouncement(viewerRole)
  return <>
    <CardHeader><div className="flex flex-wrap items-center justify-between gap-4"><div><CardTitle id="announcements-heading">Announcements</CardTitle><CardDescription>From your campus community</CardDescription></div>{canCreate && <button type="button" className={buttonClass} onClick={() => { setEditing(undefined); setOpen(true) }}>Create announcement</button>}</div></CardHeader>
    <CardContent>{announcements.length === 0 ? <p className="py-4 text-sm text-muted-foreground">{canCreate ? 'No announcements yet. Create the first one.' : 'No announcements have been posted yet.'}</p> : <ul className="space-y-4">{announcements.map((announcement, index) => <li key={announcement.id}><article><div className="flex items-center gap-2"><Badge variant={announcement.source === 'Professor' ? 'secondary' : 'outline'}>{announcement.source}</Badge><time className="text-xs text-muted-foreground" dateTime={announcement.createdAt}>{formatDate(announcement.createdAt)}</time></div><h3 className="mt-2 font-semibold">{announcement.author}</h3><p className="mt-1 text-sm leading-relaxed text-muted-foreground">{announcement.body}</p><div className="mt-3 flex gap-2"><button type="button" className={buttonClass} onClick={() => { setEditing(announcement); setOpen(true) }}><Pencil className="mr-1 inline size-3.5" aria-hidden="true" />Edit</button><button type="button" className={buttonClass} onClick={() => setDeleting(announcement)}><Trash2 className="mr-1 inline size-3.5" aria-hidden="true" />Delete</button></div></article>{index < announcements.length - 1 && <hr className="mt-4 border-border" />}</li>)}</ul>}</CardContent>
