 ...
 *
 * Every interface mirrors a table in codectory.db one-for-one (see
...
export interface AgentSession {
  ...
  /**
   * The agent's lane color from codectory.config.yaml, e.g. "#a78bfa". Null on dbs
 ...
 */
export interface QualityToolCallPayload {
  project: string;
  project_path: string;
  changed_files: string[];
  check: string;
  area?: string;
  operation: "lint" | "format" | "typecheck" | "build" | "test";
  command: string;
  returncode: number;
  passed: boolean;
...
/** One readable file from a session's context_handoff directory. */
export interface ContextHandoffFile {
  /** POSIX-style path relative to context_handoff/. */
  path: string;
  type: "markdown" | "log";
  /** Full size on disk, even when content is truncated. */
  size: number;
  content: string;
  truncated: boolean;
  truncation_reason: "file_limit" | "total_limit" | null;
}

/** GET /api/sessions/:adw_id/context-handoff */
export interface ContextHandoffResponse {
  files: ContextHandoffFile[];
  total_size: number;
  returned_bytes: number;
  truncated: boolean;
  /** Non-readable/symlink entries are counted but never followed. */
