import type { AgentStartPayload, EventRow, QualityToolCallPayload, ToolCallPayload } from './types'
...
export function parseQualityCall(e: EventRow): QualityToolCallPayload | null {
  const payload = parsePayload(e.payload_json)
  if (!payload || typeof payload.project !== 'string' || typeof payload.check !== 'string') return null
  if (typeof payload.operation !== 'string' || typeof payload.command !== 'string') return null
  if (typeof payload.passed !== 'boolean' || typeof payload.returncode !== 'number') return null
  return payload as unknown as QualityToolCallPayload
}
...
export function eventLabel(e: EventRow): string {
  if (e.type === 'tool_call') {
    ...
    const quality = parseQualityCall(e)
    if (quality) {
      return `${quality.project}/${quality.check}: ${quality.operation} · ${quality.passed ? 'passed' : 'failed'}`
    }
