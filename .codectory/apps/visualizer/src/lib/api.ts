...
export async function fetchContextHandoff(adwId: string): Promise<ContextHandoffResponse> {
  const res = await fetch(`/api/sessions/${encodeURIComponent(adwId)}/context-handoff`)
  // Older/deployed servers without the endpoint behave like sessions without handoff files.
  if (res.status === 404) {
    return { files: [], total_size: 0, returned_bytes: 0, truncated: false, skipped_entries: 0 }
  }
  if (!res.ok) throw new Error(`GET context handoff → ${res.status}`)
  const data = (await res.json()) as Partial<ContextHandoffResponse>
  return {
    files: data.files ?? [],
    total_size: data.total_size ?? 0,
    returned_bytes: data.returned_bytes ?? 0,
    truncated: data.truncated ?? false,
    skipped_entries: data.skipped_entries ?? 0,
  }
}
