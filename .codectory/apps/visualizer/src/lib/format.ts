...
// Agent tool calls carry ok:false; deterministic quality calls carry
// passed:false. Older payloads have neither key and count as ok.
export function payloadOk(raw: string | null | undefined): boolean {
  if (!raw) return true
  try {
    const p: unknown = JSON.parse(raw)
    if (p && typeof p === 'object') {
      if ('ok' in p) return (p as { ok?: unknown }).ok !== false
      if ('passed' in p) return (p as { passed?: unknown }).passed !== false
    }
  } catch {
