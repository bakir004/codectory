...
const handoff = ref<ContextHandoffResponse>({
  files: [],
  total_size: 0,
  returned_bytes: 0,
  truncated: false,
...
async function tick() {
  ...
  try {
    ...
    const now = Date.now()
    const handoffChanged = fresh.some((e) => e.type === 'handoff' || e.type === 'tool_call')
    if (!loaded.value || handoffChanged || (detail.session.status === 'running' && now - lastHandoffFetch >= 2000)) {
      lastHandoffFetch = now
      try {
        handoff.value = await fetchContextHandoff(props.adwId)
        handoffError.value = false
      } catch {
        // Handoff files are auxiliary; an unreadable tree must not break the trace.
...
const lanes = computed<Lane[]>(() => {
  ...
  for (const [i, owner] of agentOwners.entries()) {
    ...
    out.push({
      id: `agent:${owner}`,
      label: `${info?.display_name || owner} · ${owner}`,
