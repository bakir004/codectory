...
import { eventLabel, parseAgentStart, parseQualityCall, parseToolCall } from '../lib/events'
...
const props = defineProps<{
  ...
  envelopes: Envelope[]
  gates: GateResult[]
  handoff: ContextHandoffResponse
...
const qualityGroups = computed<QualityGroup[]>(() => {
  ...
  for (const event of phaseEvents.value) {
    ...
    const check = parseQualityCall(event)
    if (!check) continue
    const group = groups.get(check.project) ?? {
      project: check.project,
      projectPath: check.project_path,
      checks: [],
    }
    group.checks.push(check)
    groups.set(check.project, group)
  }
...
const openHandoffFiles = reactive(new Set<string>())

function toggleHandoffFile(path: string) {
  if (openHandoffFiles.has(path)) openHandoffFiles.delete(path)
  else openHandoffFiles.add(path)
}
    ...
    <div class="d-grid">
      ...
      >
        ...
        <DetailSection
          ...
          :open="openSections.has('handoff')"
          @toggle="toggleSection('handoff')"
        >
          <div v-if="handoffError" class="faint">context handoff unavailable</div>
          <div v-else-if="!handoff.files.length" class="faint">
            no .md or .log context handoff files recorded
          </div>
          <template v-else>
            <p v-if="handoff.truncated" class="handoff-warning">
              Display is bounded to {{ bytesLabel(handoff.returned_bytes) }} of readable content.
              <span v-if="handoff.skipped_entries">
                {{ handoff.skipped_entries }} unsafe or excess
                {{ handoff.skipped_entries === 1 ? 'entry was' : 'entries were' }} skipped.
              </span>
            </p>
            <div v-for="file in handoff.files" :key="file.path" class="handoff-file">
              <button class="handoff-head" @click="toggleHandoffFile(file.path)">
                <span class="chev">{{ openHandoffFiles.has(file.path) ? '▾' : '▸' }}</span>
                <span class="handoff-path">{{ file.path }}</span>
                <span class="tag"><span class="tag-v">{{ file.type === 'markdown' ? 'md' : 'log' }}</span></span>
                <span class="dim">{{ bytesLabel(file.size) }}</span>
              </button>
              <div v-if="openHandoffFiles.has(file.path)" class="handoff-body">
                <p v-if="file.truncated" class="handoff-warning">
                  Content truncated by the
                  {{ file.truncation_reason === 'file_limit' ? 'per-file' : 'total response' }} limit.
                </p>
                <!-- Safe: renderMarkdown escapes input; Vue escapes log interpolation. -->
                <div
                  v-if="file.type === 'markdown'"
                  class="md handoff-markdown"
                  v-html="renderMarkdown(file.content)"
                />
                <pre v-else class="handoff-log">{{ file.content }}</pre>
              </div>
            </div>
          </template>
        </DetailSection>
        ...
        >
          <div v-for="group in qualityGroups" :key="group.project" class="quality-project">
            <div class="quality-project-head">
              <strong>{{ group.project }}</strong>
              <span class="dim">{{ group.projectPath }}</span>
            </div>
            <div
            ...
            >
              <div class="quality-line">
                ...
                <span class="tag"><span class="tag-v">{{ check.operation }}</span></span>
                <span :class="check.passed ? 't-green' : 't-red'">
                  {{ check.passed ? 'passed' : `failed (${check.returncode})` }}
                </span>
              </div>
              <code class="quality-command">{{ check.command }}</code>
              <div class="quality-meta dim">
                {{ check.changed_files.length }} changed file(s)
                <span v-if="check.output_artifact"> · log: {{ check.output_artifact }}</span>
              </div>
              <details v-if="check.changed_files.length" class="quality-files">
                <summary>changed files</summary>
                <ul><li v-for="file in check.changed_files" :key="file">{{ file }}</li></ul>
              </details>
            </div>
          </div>
        </DetailSection>
...
.quality-check.pass { border-left-color: var(--green); }
.quality-check.fail { border-left-color: var(--red); }
.quality-check.pass .check-mark { color: var(--green); }
.quality-check.fail .check-mark { color: var(--red); }
