/**
 * SQLite reader over a target repo's codectory.db.
 *
...
const DEFAULT_DB_RELATIVE = "adws/adw_data/codectory.db";
const MAX_LIMIT = 1000;
const DEFAULT_LIMIT = 500;

/**
 * Resolve the db path: --db arg wins, then CODECTORY_DB, then <cwd>/adws/adw_data/codectory.db.
...
export function resolveDbPath(argv: string[] = Bun.argv): string {
  ...
  const raw =
    ...
    process.env.CODECTORY_DB ??
    DEFAULT_DB_RELATIVE;

  return isAbsolute(raw) ? raw : resolve(process.cwd(), raw);
}

export class CodectoryDb {
   ...
   * `adws/adw_data/codectory.db`), so deriving it as a sibling of the db file keeps
  ...
  constructor(path: string) {
    if (!existsSync(path)) {
      throw new Error(
        `codectory.db not found at ${path}\n` +
          `Point the visualizer at a target repo: --db <path> or CODECTORY_DB=<path>, ` +
  ...
  private agentsFor(adwIds: string[]): Map<string, AgentSession[]> {
    ...
    const displayName = this.optionalColumn("agent_sessions", "display_name");
