/**
 * CODECTORY visualizer server — JSON API over a target repo's codectory.db, plus the
 * built UI when ./dist exists. Reads are read-only; the single write is
 * POST /api/sessions/:adw_id/archive, which sets one review flag on a row.
 *
 * There is no ingest endpoint and no websocket. The data path is
 * agents → sqlite → web ui, and the UI gets there by polling.
 *
 *   bun run server/index.ts
 *   bun run server/index.ts --db /path/to/repo/adws/adw_data/codectory.db
 *   CODECTORY_DB=/path/to/codectory.db PORT=4600 bun run server/index.ts
 */
import { constants, existsSync, statSync } from "node:fs";
import { lstat, open, readdir, realpath } from "node:fs/promises";
import { join, relative, resolve, sep } from "node:path";
import { CodectoryDb, resolveDbPath } from "./db.ts";
import type {
  AgentPrompts,
  ApiError,
  ContextHandoffFile,
  ContextHandoffResponse,
  HealthResponse,
} from "../shared/types.ts";

const PORT = Number(process.env.PORT ?? 4600);
const DIST_DIR = resolve(import.meta.dir, "..", "dist");

const dbPath = resolveDbPath();
let db: CodectoryDb;
try {
  db = new CodectoryDb(dbPath);
} catch (error) {
  console.error(`[codectory] ${(error as Error).message}`);
  process.exit(1);
}

function json(data: unknown, status = 200): Response {
  return new Response(JSON.stringify(data), {
    status,
    headers: {
      "content-type": "application/json; charset=utf-8",
      "cache-control": "no-store",
    },
  });
}

function notFound(message: string): Response {
  return json({ error: message } satisfies ApiError, 404);
}

/** Guard every handler so a malformed query can't take the server down mid-run. */
function safely(
  handler: (req: Request) => Response | Promise<Response>,
): (req: Request) => Promise<Response> {
  return async (req) => {
    try {
      return await handler(req);
    } catch (error) {
      console.error(`[codectory] ${req.method} ${new URL(req.url).pathname}:`, error);
      return json({ error: (error as Error).message } satisfies ApiError, 500);
    }
  };
}

/**
 * adw_ids and agent names are path segments on disk, so anything that isn't a
 * plain identifier is rejected outright rather than sanitized into something
 * that might still escape the sessions directory.
 */
const SAFE_SEGMENT = /^[A-Za-z0-9._-]+$/;

function isSafeSegment(value: string): boolean {
  return SAFE_SEGMENT.test(value) && value !== "." && value !== "..";
}

function param(req: Request, key: string): string {
  return decodeURIComponent(
    (req as Request & { params: Record<string, string> }).params[key] ?? "",
  );
}

function intQuery(req: Request, key: string, fallback: number): number {
  const raw = new URL(req.url).searchParams.get(key);
  if (raw === null || raw.trim() === "") return fallback;
  const parsed = Number.parseInt(raw, 10);
  return Number.isFinite(parsed) ? parsed : fallback;
}

const HANDOFF_FILE_LIMIT = 256 * 1024;
const HANDOFF_TOTAL_LIMIT = 1024 * 1024;
const HANDOFF_ENTRY_LIMIT = 1000;

function isInside(root: string, candidate: string): boolean {
  return candidate === root || candidate.startsWith(root + sep);
}

/**
 * Read the bounded, human-readable portion of a session handoff tree. Symlinks
 * are never followed. O_NOFOLLOW closes the lstat/open race for files, while
 * realpath containment protects every traversed directory.
 */
async function contextHandoff(adwId: string): Promise<ContextHandoffResponse> {
  const empty: ContextHandoffResponse = {
    files: [],
    total_size: 0,
    returned_bytes: 0,
    truncated: false,
    skipped_entries: 0,
  };
  const sessionDir = resolve(db.sessionsDir, adwId);
  if (!isInside(db.sessionsDir, sessionDir)) throw new Error("session path escapes sessions root");

  try {
    const sessionStat = await lstat(sessionDir);
    if (!sessionStat.isDirectory() || sessionStat.isSymbolicLink()) {
      throw new Error("session path is not a real directory");
    }
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === "ENOENT") return empty;
    throw error;
  }

  const sessionReal = await realpath(sessionDir);
  const root = resolve(sessionDir, "context_handoff");
  try {
    const rootStat = await lstat(root);
    if (!rootStat.isDirectory() || rootStat.isSymbolicLink()) {
      throw new Error("context_handoff is not a real directory");
    }
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === "ENOENT") return empty;
    throw error;
  }
  const rootReal = await realpath(root);
  if (!isInside(sessionReal, rootReal)) throw new Error("context_handoff escapes session root");

  const files: ContextHandoffFile[] = [];
  let totalSize = 0;
  let returnedBytes = 0;
  let skippedEntries = 0;
  let visitedEntries = 0;
  let entryLimitHit = false;

  const visit = async (dir: string): Promise<void> => {
    if (entryLimitHit) return;
    const dirReal = await realpath(dir);
    if (!isInside(rootReal, dirReal)) throw new Error("handoff directory escapes root");
    const entries = await readdir(dir, { withFileTypes: true });
    entries.sort((a, b) => a.name.localeCompare(b.name));
    for (const entry of entries) {
      visitedEntries += 1;
      if (visitedEntries > HANDOFF_ENTRY_LIMIT) {
        skippedEntries += 1;
        entryLimitHit = true;
        break;
      }
      const candidate = join(dir, entry.name);
      // Sequential traversal is intentional: byte/entry budgets are shared state.
      // oxlint-disable-next-line no-await-in-loop
      const info = await lstat(candidate);
      if (info.isSymbolicLink()) {
        skippedEntries += 1;
        continue;
      }
      if (info.isDirectory()) {
        // oxlint-disable-next-line no-await-in-loop
        await visit(candidate);
        if (entryLimitHit) break;
        continue;
      }
      const ext = entry.name.toLowerCase().endsWith(".md")
        ? "markdown"
        : entry.name.toLowerCase().endsWith(".log")
          ? "log"
          : null;
      if (!info.isFile() || ext === null) continue;

      totalSize += info.size;
      const remaining = Math.max(0, HANDOFF_TOTAL_LIMIT - returnedBytes);
      const wanted = Math.min(info.size, HANDOFF_FILE_LIMIT, remaining);
      let content = "";
      if (wanted > 0) {
        // oxlint-disable-next-line no-await-in-loop
        const handle = await open(candidate, constants.O_RDONLY | constants.O_NOFOLLOW);
        try {
          const buffer = Buffer.alloc(wanted);
          // oxlint-disable-next-line no-await-in-loop
          const { bytesRead } = await handle.read(buffer, 0, wanted, 0);
          returnedBytes += bytesRead;
          content = new TextDecoder("utf-8", { fatal: false }).decode(buffer.subarray(0, bytesRead));
        } finally {
          // oxlint-disable-next-line no-await-in-loop
          await handle.close();
        }
      }
      const fileLimited = info.size > HANDOFF_FILE_LIMIT && HANDOFF_FILE_LIMIT <= remaining;
      const truncated = wanted < info.size;
      files.push({
        path: relative(root, candidate).split(sep).join("/"),
        type: ext,
        size: info.size,
        content,
        truncated,
        truncation_reason: truncated ? (fileLimited ? "file_limit" : "total_limit") : null,
      });
    }
  };

  await visit(root);
  return {
    files,
    total_size: totalSize,
    returned_bytes: returnedBytes,
    truncated: files.some((file) => file.truncated) || entryLimitHit,
    skipped_entries: skippedEntries,
  };
}

/** Serve the built SPA if it has been built; otherwise point at the dev server. */
async function serveStatic(req: Request): Promise<Response> {
  const { pathname } = new URL(req.url);

  if (!existsSync(DIST_DIR)) {
    return new Response(
      `CODECTORY visualizer API is running on :${PORT}.\n\n` +
        `No ./dist build found. Run "bun run dev" for the Vite dev server ` +
        `(it proxies /api here), or "bun run build" to serve the UI from this process.\n`,
      { status: 200, headers: { "content-type": "text/plain; charset=utf-8" } },
    );
  }

  // Reject traversal before touching the filesystem.
  const candidate = resolve(join(DIST_DIR, pathname));
  if (candidate === DIST_DIR || candidate.startsWith(DIST_DIR + "/")) {
    if (existsSync(candidate) && statSync(candidate).isFile()) {
      return new Response(Bun.file(candidate));
    }
  }

  // SPA fallback: breadcrumb routes are client-side.
  const indexHtml = join(DIST_DIR, "index.html");
  if (existsSync(indexHtml)) {
    return new Response(Bun.file(indexHtml), {
      headers: { "content-type": "text/html; charset=utf-8" },
    });
  }
  return notFound("not found");
}

const server = Bun.serve({
  port: PORT,
  routes: {
    "/api/health": safely(
      () =>
        json({
          ok: true,
          db: db.path,
          journal_mode: db.journalMode,
          sessions: db.sessionCount(),
        } satisfies HealthResponse),
    ),

    "/api/sessions": safely((req) => json(db.sessions(intQuery(req, "limit", 200)))),

    "/api/sessions/:adw_id": safely((req) => {
      const detail = db.sessionDetail(param(req, "adw_id"));
      return detail ? json(detail) : notFound(`no session ${param(req, "adw_id")}`);
    }),

    // The one write. Archiving is review triage — it belongs to the reader, not
    // to the run — so it never touches anything a tracer wrote.
    "/api/sessions/:adw_id/archive": {
      POST: safely(async (req) => {
        const adwId = param(req, "adw_id");
        if (!isSafeSegment(adwId)) {
          return json({ error: "invalid adw_id" } satisfies ApiError, 400);
        }
        const body = (await req.json().catch(() => ({}))) as { archived?: unknown };
        const archived = body.archived === undefined ? true : Boolean(body.archived);
        return db.setArchived(adwId, archived)
          ? json({ adw_id: adwId, archived })
          : notFound(`no session ${adwId}`);
      }),
    },

    "/api/sessions/:adw_id/events": safely((req) =>
      json(
        db.events(
          param(req, "adw_id"),
          intQuery(req, "after", 0),
          intQuery(req, "limit", 500),
        ),
      ),
    ),

    "/api/sessions/:adw_id/envelopes": safely((req) =>
      json(db.envelopes(param(req, "adw_id"))),
    ),

    "/api/sessions/:adw_id/gates": safely((req) => json(db.gates(param(req, "adw_id")))),

    "/api/sessions/:adw_id/context-handoff": safely(async (req) => {
      const adwId = param(req, "adw_id");
      if (!isSafeSegment(adwId)) {
        return json({ error: "invalid adw_id" } satisfies ApiError, 400);
      }
      if (!db.session(adwId)) return notFound(`no session ${adwId}`);
      return json(await contextHandoff(adwId));
    }),

    // The exact prompts an agent was sent, read from the session dir. Files are
    // the raw record; the db has no copy of them.
    "/api/sessions/:adw_id/agents/:agent/prompts": safely(async (req) => {
      const adwId = param(req, "adw_id");
      const agent = param(req, "agent");
      if (!isSafeSegment(adwId) || !isSafeSegment(agent)) {
        return json({ error: "invalid adw_id or agent" } satisfies ApiError, 400);
      }
      if (!db.session(adwId)) return notFound(`no session ${adwId}`);

      const dir = resolve(db.sessionsDir, adwId, agent, "prompts");
      // Defense in depth: the segment check already forbids traversal.
      if (dir !== db.sessionsDir && !dir.startsWith(db.sessionsDir + sep)) {
        return json({ error: "invalid path" } satisfies ApiError, 400);
      }

      // A prompt file is absent whenever the agent never ran in this session —
      // a normal state, so it reads as null rather than an error.
      const read = async (name: string): Promise<string | null> => {
        const file = Bun.file(join(dir, `${name}.md`));
        return (await file.exists()) ? await file.text() : null;
      };
      return json({
        system: await read("system"),
        user: await read("user"),
      } satisfies AgentPrompts);
    }),
  },

  fetch(req) {
    const { pathname } = new URL(req.url);
    if (pathname.startsWith("/api/")) return notFound(`no route ${pathname}`);
    return serveStatic(req);
  },
});

console.log(`[codectory] visualizer api  http://localhost:${server.port}`);
console.log(`[codectory] db              ${db.path}  [journal_mode=${db.journalMode}]`);
console.log(
  existsSync(DIST_DIR)
    ? `[codectory] serving ui from  ${DIST_DIR}`
    : `[codectory] no ./dist — use "bun run dev" for the Vite dev server on :4601`,
);

process.on("SIGINT", () => {
  db.close();
  process.exit(0);
});
