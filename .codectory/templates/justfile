# CODECTORY starter recipes. Stamped by install.py, then yours to edit.
#
...
# Every recipe passes this through, so `CODECTORY_CONFIG=other.yaml just sdlc "..."`
# swaps the whole roster for one run.
config := env_var_or_default("CODECTORY_CONFIG", "adws/adw_codectory_config/codectory.config.yaml")
...
obs:
    cd .codectory/apps/visualizer && bun install && (CODECTORY_DB={{justfile_directory()}}/{{db}} bun run server/index.ts &) && bunx vite
