...
Usage:
    uv run adws/adw_plan_build_test.py "<prompt or path/to/prompt.md>" [--config adws/adw_codectory_config/codectory.config.yaml] [--adw-id a1b2c3d4]

Phases: engineer(request) -> planner -> builder -> code(test) [-> builder(fix) -> code(test) ... bounded] -> git(commit)

Testing is CODE: project test commands live in projects.yaml, so no agent
spends a context window rediscovering them. Failures flow back to the
...
def main(prompt: str, config: str = "adws/adw_codectory_config/codectory.config.yaml", adw_id: str | None = None) -> int:
    ...
    for i in range(1, MAX_FIX_LOOPS + 2):
        ...
        if test.passed or i > MAX_FIX_LOOPS:
            break
    ...
    if test is not None and test.passed:
        with run.phase(PhaseParams(name="commit", kind="code", owner="git",
            ...
            message = previous.commit_message or f"codectory({run.adw_id}): {previous.summary}"
