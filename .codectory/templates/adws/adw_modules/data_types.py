"""Concrete data types for the CODECTORY ADW system.
...
QualityOperation = Literal["lint", "format", "typecheck", "build", "test"]
QualityScope = Literal["changed", "all"]


def _relative_repo_path(value: str) -> str:
    """Normalize a configured POSIX path and reject paths outside the repo."""
    text = value.strip().replace("\\", "/")
    path = PurePosixPath(text or ".")
    if path.is_absolute() or ".." in path.parts:
        raise ValueError("must be a relative path that stays inside the repository")
    normalized = path.as_posix()
    return "." if normalized in ("", ".") else normalized.removeprefix("./")


class ProjectCheckConfig(BaseModel):
    """One command declared in projects.yaml."""
    ...
    timeout_seconds: int = Field(
        default=120, gt=0, validation_alias=AliasChoices("timeout_seconds", "timeout"))
    ...
    def _non_empty_label(cls, value: str) -> str:
        ...
        if "/" in value or "\\" in value or value in (".", ".."):
            raise ValueError("must be a safe label without path separators")
        return value

    @field_validator("command")
    @classmethod
    def _argv_is_non_empty(cls, value: list[str]) -> list[str]:
        if (not value or any(not isinstance(item, str) or not item for item in value)
                or not value[0].strip()):
            raise ValueError("must be a non-empty YAML array with a real executable argv[0]")
        return list(value)


class ProjectConfig(BaseModel):
    """A repository project and the deterministic checks run from its directory."""

    model_config = ConfigDict(extra="forbid")

    name: str
    path: str = Field(default=".", validation_alias=AliasChoices("path", "relative_path"))
    guides: str = Field(default="guides", validation_alias=AliasChoices("guides", "guides_path"))
    checks: list[ProjectCheckConfig] = Field(min_length=1)
    ...
    def _safe_name(cls, value: str) -> str:
        ...
        if not value or "/" in value or "\\" in value or value in (".", ".."):
            raise ValueError("must be a safe non-empty label without path separators")
        return value

    @field_validator("path", "guides")
    @classmethod
    def _safe_relative_path(cls, value: str) -> str:
        return _relative_repo_path(value)

    @model_validator(mode="after")
    def _unique_check_names(self) -> "ProjectConfig":
...
class ProjectsConfig(BaseModel):
    ...
    @model_validator(mode="after")
    def _unique_project_names(self) -> "ProjectsConfig":
        names = [project.name for project in self.projects]
        duplicates = sorted({name for name in names if names.count(name) > 1})
        if duplicates:
            raise ValueError(f"duplicate project name(s): {', '.join(duplicates)}")
        return self


class QualityCheckSpec(BaseModel):
    """A configured command bound to its project and changed-file evidence."""
...
class ConfigDefaults(BaseModel):
    ...
    protected_files: list[str] = Field(default_factory=lambda: [
        "adws/adw_modules/", "adws/adw_codectory_config/", "adws/adw_*.py",
    ])
    data_dir: str = "adws/adw_data"


class ObservabilityConfig(BaseModel):
    db: str = "adws/adw_data/codectory.db"
    poll_ms: int = 500


class CODECTORYConfig(BaseModel):
    defaults: ConfigDefaults = Field(default_factory=ConfigDefaults)
    observability: ObservabilityConfig = Field(default_factory=ObservabilityConfig)
    agents: list[AgentConfig] = Field(default_factory=list)
    # Runtime-only origin used to resolve sibling configuration such as
    # projects.yaml. Excluded from serialization so it never becomes config data.
