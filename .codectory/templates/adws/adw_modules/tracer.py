...
Files are the raw record; codectory.db is the queryable mirror the UI polls.
...
MIGRATIONS = [("agent_sessions", "display_name", "TEXT"),
...
class Tracer:
    ...
    def _migrate(self) -> None:
        """Additive column migrations, so a db from an older CODECTORY still opens."""
    ...
    def agent_session_row(self, adw_id: str, agent: AgentConfig, session_id: str,
        ...
        ts = now_iso()
        self.conn.execute(
            "INSERT INTO agent_sessions (adw_id, agent, display_name, coding_agent, model, color,"
            ...
            (adw_id, agent.name, agent.display_name or agent.name, agent.coding_agent,
             agent.model, agent.color, session_id, context_tokens, context_window, ts, ts),
        )
