...
Usage:
    uv run adws/adw_build_test.py "<prompt or path/to/prompt.md>" [--config adws/adw_codectory_config/codectory.config.yaml] [--adw-id a1b2c3d4]

Phases: engineer(request) -> builder -> code(test) [-> builder(fix) -> code(test) ... bounded]

Testing is CODE. Test commands are declared per project in projects.yaml,
so running them needs no judgement — only repairing them does. Failures reach the
