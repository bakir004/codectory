...
```
adws/adw_codectory_config/codectory.config.yaml
    planner     fireworks/accounts/fireworks/models/kimi-k3
    builder     google/gemini-3.6-flash (inherited)
adws/adw_codectory_config/codectory.frontier.config.yaml
...
| nothing about models at all | the default, `adws/adw_codectory_config/codectory.config.yaml` |
...
The trace db is `adws/adw_data/codectory.db`. It is WAL, so reads never block the running writers — poll it as often as you like.

```bash
# where the run stands
sqlite3 adws/adw_data/codectory.db \
