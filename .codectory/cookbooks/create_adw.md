...
1. **What agents, in what order?** Pick from the roster (`adws/adw_codectory_config/codectory.config.yaml`). The starter six cover most chains:

| Agent | Use when | Output type | Typical gates |
|---|---|---|---|
| `research` | current library/framework/API documentation is required | `ResearchOutput` | `artifacts_exist`, `files_non_empty` |
...
```bash
uv run .claude/skills/codectory/scripts/make_adw.py --name review_docs --agents scout,builder
```
...
def main(prompt: str, config: str = "adws/adw_codectory_config/codectory.config.yaml", adw_id: str | None = None) -> int:
    ...
    with run.phase(PhaseParams(name="commit", kind="code", owner="git",
        ...
        message = build.commit_message or f"codectory({run.adw_id}): {build.summary}"
