...
The system map the orchestrator reads on startup — what CODECTORY is, how a stamped repo is laid out, and which cookbook to load next.

## What CODECTORY is

Codectory builds repeatable **agents plus code** workflows. Deterministic Python (an ADW script) owns sequencing, retries, and acceptance; agents are bounded nodes inside that graph. Agent proposes, code disposes.
...
```
projects.yaml                       project paths + deterministic quality commands
adws/
├── adw_codectory_config/
│   └── codectory.config.yaml         the agent roster — one agent, one prompt, one purpose
...
│   ├── quality.py               projects.yaml → changed-project checks → repair envelope
...
└── adw_data/
    ...
    └── codectory.db                  gitignored SQLite trace db the visualizer polls
```
...
```bash
uv run adws/adw_plan.py "add a /health endpoint" --projects api
uv run adws/adw_plan_build.py requests/health.md --projects api --adw-id a1b2c3d4
```
...
| Generate `codectory.config.yaml` | `create_config.md` |
| Add or retune an agent | `update_config.md` |
| Add low-level logic or a gate | `update_modules.md` |
| Configure lint, format, typecheck, build, or tests | `configure_quality.md` |
