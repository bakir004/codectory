...
`/codectory install` — stamp the entire factory out of the skill and into the current working directory.

## Run it

```bash
uv run .claude/skills/codectory/scripts/install.py
```

Run from the **target repo root** — the cwd is where everything lands. If the skill lives in your user scope, the path is `~/.claude/skills/codectory/scripts/install.py`.
...
|---|---|---|
| `adws/adw_codectory_config/codectory.config.yaml` | `templates/codectory.config.yaml` | yes — the agent roster |
...
| `adws/adw_data/sessions/`, `adws/adw_data/codectory.db` | created at runtime | no — gitignored |
...
4. **Gitignore** — `install.py` appends `adws/adw_data/sessions/`, `adws/adw_data/codectory.db*`, and `.env` for you; confirm they landed. All three are runtime or secrets and must never be committed.
...
```bash
sqlite3 adws/adw_data/codectory.db "select adw_id, status from sessions order by started_at desc limit 1;"
```
