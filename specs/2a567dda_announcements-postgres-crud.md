...
- Configure both Drizzle commands and the TanStack Start server runtime to consume the root environment reliably when commands are launched from `web/`. Keep database access server-only. Document any required working directory assumptions rather than relying on accidental shell environment state.
