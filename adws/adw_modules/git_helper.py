...
def worktree_fingerprint() -> str:
    """Content-sensitive snapshot used to decide whether a revision changed files."""
    diff = subprocess.run(["git", "diff", "--binary", "HEAD"], capture_output=True)
    if diff.returncode != 0:
        raise RuntimeError(f"git diff --binary HEAD failed: {diff.stderr.decode(errors='replace').strip()}")
