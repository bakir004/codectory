...
def load_config(path: str = "adws/adw_codectory_config/codectory.config.yaml") -> CODECTORYConfig:
    config_path = Path(path)
    raw = yaml.safe_load(config_path.read_text()) or {}
    agents_path = config_path.with_name("agents.yaml")
    if agents_path.is_file():
        agent_raw = yaml.safe_load(agents_path.read_text()) or {}
        raw["agents"] = agent_raw.get("agents", [])
    ...
    config = CODECTORYConfig(**raw)
    config.config_path = str(config_path.resolve())
    return config


def resolve(cfg: CODECTORYConfig, name: str) -> AgentConfig:
...
def validate(cfg: CODECTORYConfig, required: list[str]) -> None:
