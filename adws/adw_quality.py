...
# ///
"""ADW Quality — run configured checks for every changed project.

Usage:
    uv run adws/adw_quality.py "<reason for the quality run>" [--config adws/adw_codectory_config/codectory.config.yaml] [--adw-id a1b2c3d4]
...
def main(prompt: str, config: str = "adws/adw_codectory_config/codectory.config.yaml", adw_id: str | None = None) -> int:
    ...
    with run.phase(PhaseParams(name="quality", kind="code", owner="quality",
                               description="Run projects.yaml checks for changed projects")) as ph:
